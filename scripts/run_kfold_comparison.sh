...
SPLIT_DIR="${SPLIT_DIR:-$PKG_ROOT/dataset/splits/kfold_second_setting}"
RUN_ROOT="${RUN_ROOT:-$PKG_ROOT/engine/runs/kfold_second_setting_20260529}"
...
run_fold() {
  ...
  run_cladapter "fold${fold}_convnextb_cla_sft2" "$gpu_id" \
    ...
    --backbone-out-dim 1024 \
    --backbone-num-patch 49 \
    --finetune-ckpt "$fold_root/convnextb_cla_stage1/convnext_base.clip_laion2b_augreg_final.pth" \
    --norm clip \
  ...
  run_cladapter "fold${fold}_vitb_cla_sft2" "$gpu_id" \
    ...
    --backbone-out-dim 768 \
    --backbone-num-patch 196 \
    --finetune-ckpt "$fold_root/vitb_cla_stage1/vit_base_patch16_clip_224.laion2b_final.pth" \
    --norm clip \
    --output-dir "$fold_root/vitb_cla_sft2"
}

"$SCRIPT_DIR/make_kfold_group_train_test_splits.py" \
...
"$SCRIPT_DIR/summarize_kfold_results.py" \
  --run-root "$RUN_ROOT" \
  --split-dir "$SPLIT_DIR" \
  --output-md "$RUN_ROOT/RESULTS.md"

echo "All k-fold comparison runs completed under $RUN_ROOT"
